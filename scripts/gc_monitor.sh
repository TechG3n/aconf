...
#Create logfile, stolen from gc.sh
if [ ! -e /sdcard/gc_monitor.log ] ;then
	touch /sdcard/gc_monitor.log
fi
	...
	curl -k -X POST $atvdetails_receiver_host:$atvdetails_receiver_port/webhook -H "Accept: application/json" -H "Content-Type: application/json" --data-binary @- <<DATA
        {
            ...
            "issue": "${issue}",
            "action": "${action}",
            "script": "gc_monitor.sh"
        }
DATA
}


echo "`date +%Y-%m-%d_%T` [MONITORBOT] Starting gc data monitor in 5 mins, loop is $monitor_interval seconds" >> $logfile
sleep 300
while :
do
	[[ $useMonitor == "false" ]] && echo "`date +%Y-%m-%d_%T` gc_monitor stopped" >> $logfile && exit 1
	...
	[[ -z $origin ]] && origin=$(cat $aconf | tr , '\n' | grep -w 'device_name' | awk -F "\"" '{ print $4 }')
		...
		echo  "`date +%Y-%m-%d_%T` [MONITORBOT] Checking gc and Pogo for update" >> $logfile
		updatecheck=0
		check_for_updates
	fi
