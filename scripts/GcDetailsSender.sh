...
#Configs
gc_conf="/data/local/tmp/cos/gc.json"
gc_log="/data/local/tmp/cos/gc.log"
aconf_log="/sdcard/aconf.log"
monitor_log="/sdcard/gc_monitor.log"
...
# generic
    ...
    deviceName=$(cat $gc_conf | tr , '\n' | grep -w 'device_name' | awk -F "\"" '{ print $4 }')
    arch=$(uname -m)
    productmodel=$(getprop ro.product.model)
    MITMSh=$(head -2 /system/bin/gc.sh | grep '# version' | awk '{ print $NF }')
    MITM55=$([ -f /system/etc/init.d/55gc ] && head -2 /system/etc/init.d/55gc | grep '# version' | awk '{ print $NF }' || echo 'na')
...
# gc config
    authBearer=$(cat $gc_conf | tr , '\n' | grep -w 'authBearer' | awk -F ":" '{ print $2 }' | tr -d \"})
    token=$(cat $gc_conf | tr , '\n' | grep -w 'deviceAuthToken' | awk -F ":" '{ print $2 }' | tr -d \"})
    email=$(cat $gc_conf | tr , '\n' | grep -w 'email' | awk -F ":" '{ print $2 }' | tr -d \"})
    rdmUrl=$(cat $gc_conf | tr , '\n' | grep -w 'rdmUrl' | awk -F "\"" '{ print $4 }')
    onBoot=$(cat $gc_conf | tr , '\n' | grep -w 'runOnBoot' | awk -F ":" '{ print $2 }' | tr -d \"})
# gc.log
    a_pogoStarted=$(grep 'Launched Pokemon Go' $gc_log | wc -l)
    a_injection=$(grep 'Injected successfully' $gc_log | wc -l)
    a_ptcLogin=$(grep 'Logged in using ptc' $gc_log | wc -l)
    a_MITMCrash=$(grep 'Agent has crashed or stopped responding' $gc_log | wc -l)
    a_rdmError=$(grep 'Could not send heartbeat' $gc_log | wc -l)

# monitor.log
    m_noInternet=$(grep 'No internet' $monitor_log | wc -l)
    m_noConfig=$(grep 'gc_config.json does not exist or is empty' $monitor_log | wc -l)
    m_noLicense=$(grep 'Device Lost gc License' $monitor_log | wc -l)
    m_MITMDied=$(grep 'gc must be dead, rebooting device' $monitor_log | wc -l)
    m_pogoDied=$(grep 'Pogo must be dead, rebooting device' $monitor_log | wc -l)
    m_deviceOffline=$(grep 'Device must be offline. Running a stop mapping service of gc, killing pogo and clearing junk' $monitor_log | wc -l)
